{ config, lib, pkgs, ... }: {
  config = {
    ...
    nixpkgs.config.rocmSupport =
      false; # gfx1150 has no rocm support, see https://rocm.docs.amd.com/en/latest/compatibility/compatibility-matrix.html
    home-manager.sharedModules =
      [{ home.packages = with pkgs; [ vulkan-tools nvtopPackages.amd ]; }];
  };
}
