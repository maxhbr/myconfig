    ...
    { systemd.packages = [ pkgs.systemd-cryptsetup-generator ];
      ...
      fileSystems."/mnt/4x500" =
        { device = "/dev/mapper/4x500-0";
          fsType = "btrfs";
          options = [ "defaults" "noatime" "compress=zstd" "subvol=@sub" "nofail" "x-systemd.requires-mounts-for=/mnt/.4x500-3" ];
        };
      ...
      fileSystems."/mnt/.4x500-2" =
        { device = "/dev/mapper/4x500-2";
          fsType = "btrfs";
          options = [ "defaults" "noatime" "compress=zstd" "subvol=@sub" "nofail" "x-systemd.requires-mounts-for=/mnt/.4x500-1" ];
        };
      fileSystems."/mnt/.4x500-3" =
        { device = "/dev/mapper/4x500-3";
          fsType = "btrfs";
          options = [ "defaults" "noatime" "compress=zstd" "subvol=@sub" "nofail" "x-systemd.requires-mounts-for=/mnt/.4x500-2" ];
        };
