...
in
{
        ...
        home-manager.sharedModules = [
          {
            programs.fish = {
              shellAbbrs = {
                nix-flake = "${nix}/bin/nix --experimental-features 'nix-command flakes' flake";
              };
            };
          }
        ];
  ...
  config = {
        ...
        imports = [
          {
            programs.fish = {
              ...
              functions = {
                nixTest = "NIXPKGS_ALLOW_UNFREE=1 ${nix}/bin/nix-shell '<nixpkgs>' --fallback --run fish -p $argv";
                # see: https://github.com/NixOS/nixpkgs/issues/51368#issuecomment-704678563
                nix-closure-size = "${nix}/bin/nix-store -q --size (${nix}/bin/nix-store -qR (readlink -e $argv) ) | awk '{ a+=$1 } END { print a }' | ${pkgs.coreutils}/bin/numfmt --to=iec-i";
                # see: https://nixos.wiki/wiki/Nix_command/path-info
                nix-closure-sizes = "${nix}/bin/nix path-info -rS (readlink -e $argv) | sort -nk2";
                nix-most-recently-added = "${nix}/bin/nix path-info --json --all | ${pkgs.jq}/bin/jq -r 'sort_by(.registrationTime)[-11:-1][].path'";
                nix-list-big-closures = "${nix}/bin/nix path-info --json --all -S | ${pkgs.jq}/bin/jq -r 'map(select(.closureSize > 1e9)) | sort_by(.closureSize) | map([.path, .closureSize]) | @csv'";
              };
            };
          }
        ];
    ...
    nix = {
      ...
      package =
        if lib.versionAtLeast pkgs.nix.version "2.29.0" then pkgs.nix else pkgs.nixVersions.nix_2_29;
