...
let
  ...
  myborgbackup =
    pkgs.runCommandLocal "myborgbackup.sh"
      {
      ...
      ''
        ...
        wrapProgram "$out/bin/myborgbackup.sh" --prefix PATH : ${pkgs.lib.makeBinPath myborgbackup-deps}
      '';

  backupLuksHeader-deps = with pkgs; [ cryptsetup ];
  backupLuksHeader =
    pkgs.runCommandLocal "backupLuksHeader.sh"
      {
        nativeBuildInputs = [ pkgs.makeWrapper ];
      }
      ''
        ...
        wrapProgram "$out/bin/backupLuksHeader.sh" --prefix PATH : ${pkgs.lib.makeBinPath backupLuksHeader-deps}
      '';
