...
rendezvous and never contacted the LAN host directly — yet the `/32`
shadowed the rendezvous `10.199.199.0/24` catch-all (longest-prefix
match), so replies were cryptokey-routed to an endpoint-less peer and
silently dropped. Ghosting the roaming peer symmetrically lets those
replies fall through to the rendezvous relay in both directions. The
**reverse roaming probe** (below) promotes the peer back to a direct
`/32` when it's genuinely on the home LAN.
...
ghost peers. But a roaming peer is now ghosted **symmetrically**, so the
LAN-anchored host also needs a way to notice "this roaming peer is
actually on my LAN right now" and promote it back to a direct `/32`.
That's the reverse probe, generated by `mkReverseRoamingScript` and
wired as `wg-roaming-reverse-<wg>.service` + `.timer` on non-roaming
hosts that have at least one roaming peer on their LAN.

**Candidates** — `getReverseRoamingPeersFor <wg> <thisHost>` returns
every roaming peer that (a) isn't this host, (b) isn't the rendezvous,
(c) shares this host's `network`, and (d) has
`wireguard.<wg>.{ip4,pubkey}`. No LAN ip is baked in: a roaming peer's
`ip4` is a snapshot, so the endpoint is *learned* at runtime.

**Detection** — for each candidate pubkey the script inspects:

```bash
wg show <wg> latest-handshakes   # handshake age
wg show <wg> endpoints           # learned outer address
```

and promotes the peer only when **both** hold:

* handshake age `< 180s` (a fresh *direct* handshake, i.e. the peer is
  reaching us without the rendezvous relay), and
* the learned endpoint IP is inside this host's LAN CIDR
  (`lanCidrFor <thisHost>`: `metadata.networks.<net>.cidr` if present,
  else a `/24` inferred from `defaultGateway`).

**Application**:

* promote: `wg set <wg> peer <pk> allowed-ips <wgIp>/32`
* demote:  `wg set <wg> peer <pk> allowed-ips ""` (fall back to the
  rendezvous catch-all)

No endpoint is ever set here — WireGuard endpoint roaming already pins
the peer's outer address from its inbound packets.

**Idempotency** — `/run/wg-roaming/<wg>-reverse.<pk-sanitized>` records
the last applied state (`on`/`off`) per peer; the script only `wg set`s
on a flip, mirroring the forward probe. The pubkey is base64 and can
contain `/` and `+`, which are unsafe in a filename (a `/` would be
treated as a directory separator and break the state write), so it is
sanitized to `[A-Za-z0-9_]` before building the path.

**Stale-promotion window** — promotion holds while the handshake age is
`< 180s` *and* the last-learned endpoint is inside the LAN CIDR. When a
roaming peer leaves the LAN it stops sending us direct packets, so its
learned endpoint stays pinned to its old LAN IP and its handshake can
stay "fresh" for up to 180s. During that window this host keeps the
direct `/32` pointed at a now-dead LAN endpoint, black-holing
`host → peer` traffic instead of relaying via the rendezvous. It
self-heals after ≤180s (the handshake goes stale → the probe demotes the
peer back to a ghost → traffic falls through to the rendezvous
catch-all). The 180s threshold is a deliberate trade-off between
tolerating brief packet loss and shortening this window; lowering it
makes promotion flap more on a lossy LAN.

**Triggers** — `wantedBy = [ "multi-user.target" ]`, ordered
after/requires `wireguard-<wg>.service`, plus a timer
(`OnBootSec = 45s; OnUnitActiveSec = 60s; AccuracySec = 5s`).
...
# Machine-readable status snapshot, refreshed by every probe run.
# Only roaming hosts publish this file.
cat /run/wg/wg0.status.json
# {"interface":"wg0","up":true,"mode":"on-lan","roaming":true,
#  "peers_total":10,"peers_handshake_recent":4,"ts":1730000000}
...
Roaming hosts publish `/run/wg/<iface>.status.json` on every probe run.
The schema:

| Field                     | Type   | Description                                                |
| ------------------------- | ------ | ---------------------------------------------------------- |
| `interface`               | string | Interface name (`wg0`)                                     |
| `up`                      | bool   | Whether `wg show <iface>` succeeds                         |
| `mode`                    | string | `"on-lan"` or `"off-lan"` (matches `<iface>.mode` file)    |
| `roaming`                 | bool   | Always `true` for hosts that publish the file              |
| `peers_total`             | int    | Total peers configured on the interface                    |
| `peers_handshake_recent`  | int    | Peers with a handshake within the last 180s                |
| `ts`                      | int    | Unix timestamp of the last update                          |

Atomic write via `mv` of a `.tmp` sibling, so readers never see a torn
JSON object.

### Waybar indicator

`myconfig.wireguard.<wg>.waybar.enable` (default `true`) adds a
`custom/wg-<iface>` entry to the waybar `mainBar` `modules-center`
section on roaming hosts whose waybar is enabled. The reader script is
generated by this module (`mkWaybarReader`) and uses `jq` to derive
text + class from the status file. Click the indicator to trigger an
immediate `wg-roaming-<iface>.service` run.

States rendered (text → class):

| Condition                              | Text     | Class       |
| -------------------------------------- | -------- | ----------- |
| `up = false`                           | `wg ✗`   | `error`     |
| Status file older than 180s            | `wg LAN` / `wg VPN` | `warning` |
| `mode = on-lan`                        | `wg LAN` | _(default)_ |
| `mode = off-lan`                       | `wg VPN` | `warning`   |
| Status file missing (boot race)        | `wg ?`   | `warning`   |

The existing `.warning` / `.error` classes in `waybar.gtk.css` provide
the styling for free.
...
* **Why per-key `lib.mkMerge` instead of one big `config = ...`**.
  ...
  *other* keys exist.
* **Status file is roaming-only.** Non-roaming hosts don't publish
  `/run/wg/<iface>.status.json`. The roaming/off-LAN distinction is
  the only state the indicator carries, and on a stationary
  LAN-anchored host that distinction is meaningless — the status would
  always be one fixed value. Skipping the file (and the timer that
  would refresh it) keeps the module's runtime footprint zero on
  hosts that don't need it.
