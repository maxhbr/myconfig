        ...
        home.packages = [
          ...
          (pkgs.writeShellApplication {
            ...
            text = ''
              ...
              # Usage: $cleanup_script [--merge] [--force|-f]
              force=0
              merge=0
              for a in "\$@"; do
                case "\$a" in
              ...
              if [ "\$merge" -eq 0 ]; then
                read -r -p "Merge \$branch into the main branch before cleanup? [y/N] " answer
                case "\$answer" in
                  [yY]|[yY][eE][sS]) merge=1 ;;
                esac
              fi
              ...
              cleanup_on_exit() {
                ...
                trap - EXIT
                if [ -t 0 ]; then
                  printf 'Pi session ended. Cleanup worktree? [y]es, [m]erge and cleanup, [N]o: '
                  if read -r answer; then
                    case "$answer" in
                      [yY]|[yY][eE][sS]) "$cleanup_script" || true ;;
