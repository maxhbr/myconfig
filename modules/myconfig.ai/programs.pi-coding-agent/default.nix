...
let
  ...
  # built-in `light` theme and recolor the editor box border so it turns green
  # when jailed and red when not, while keeping the rest of the theme intact.
  #
  ...
  # Green border when sandboxed, red border when not.
  ...
  jailMarkerExtension = pkgs.writeText "pi-jail-marker.ts" ''
    // Auto-generated by myconfig.ai.pi-coding-agent. Do not edit by hand.
    // Switches to the `jailed` (green border) theme when pi runs inside the
    // `jailed-pi` sandbox (PI_JAIL_MARKER=1), or the `unjailed` (red border)
  ...
  jailed-pi = jail-app {
    ...
    # below) reads this variable and switches to the green-border `jailed`
    # theme when set (vs. the red-border `unjailed` theme otherwise).
