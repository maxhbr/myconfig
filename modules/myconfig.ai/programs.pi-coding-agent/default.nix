        ...
        home.packages = [
          ...
          (pkgs.writeShellApplication {
            ...
            text = ''
              ...
              # re-exports `PI_WORKTREE_MAIN_REPO` (the jail needs it to
              # re-bind the main repo read-only, see `jail-app.nix`
              # `extraReadOnlyEnvPaths`; without it git inside the resumed
              # jail fails with `fatal: not a git repository`), `cd`s to its
              # own directory (so it works regardless of where it is invoked
              # from) and `exec`s `jailed-pi -c` to continue the most recent
              # session for this cwd (sessions are keyed by working
              # directory, and this script lives in the worktree, so `-c`
              # resumes the session originally started by
              # `jailed-pi-worktree`).
              #
              ...
              cd "\$script_dir"
              # Re-export so the jail re-binds the main repo read-only (see jail-app.nix
              # extraReadOnlyEnvPaths). Without this, git inside the jail fails:
