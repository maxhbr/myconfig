...
let
  ...
  # built-in `light` theme and recolor the *footer* font so it turns green
  # when jailed and red when not, while keeping the rest of the theme (and the
  # footer's layout with git stats etc.) intact.
  #
  # The footer (see `dist/modes/interactive/components/footer.js`) renders its
  # text with the `dim` token (bulk of the line) and `muted` for secondary
  # bits, so we override those two tokens with the marker color.
  #
  ...
  # Produce a forked theme: same as `light` but with the footer font color
  # (`dim` / `muted` tokens) overridden to `color`, and renamed to `name`.
  mkForkedTheme =
    {
      name,
      color,
    ...
    let
      forked = baseTheme // {
        inherit name;
        # Inject the marker color as a var, then point the footer text tokens
        # at it.
  ...
  # Green footer when sandboxed, red footer when not.
  ...
  jailMarkerExtension = pkgs.writeText "pi-jail-marker.ts" ''
    // Auto-generated by myconfig.ai.pi-coding-agent. Do not edit by hand.
    // Switches to the `jailed` (green footer) theme when pi runs inside the
    // `jailed-pi` sandbox (PI_JAIL_MARKER=1), or the `unjailed` (red footer)
    // theme otherwise. Both themes are forks of the built-in `light` theme
    // deployed to ~/.pi/agent/themes/.
    import type { ExtensionAPI } from "@mariozechner/pi-coding-agent";
  ...
  jailed-pi = jail-app {
    ...
    # below) reads this variable and switches to the green-footer `jailed`
    # theme when set (vs. the red-footer `unjailed` theme otherwise).
