...
let
  ...
  # extension reads it and overrides the footer with a colored marker:
  #   * inside the jail  -> "success" color (good / safe)
  #   * outside the jail -> "warning" color (unsandboxed)
  jailMarkerExtension = pkgs.writeText "pi-jail-marker.ts" ''
    // Auto-generated by myconfig.ai.pi-coding-agent. Do not edit by hand.
    // Recolors the footer to indicate whether pi is running inside the
    // `jailed-pi` sandbox (PI_JAIL_MARKER=1 -> "success" / good) or not
    // (-> "warning").
    import type { ExtensionAPI } from "@mariozechner/pi-coding-agent";

    export default function (pi: ExtensionAPI) {
      const jailed = process.env.PI_JAIL_MARKER === "1";
      const color = jailed ? "success" : "warning";
      const text = jailed ? "\u{1F512} JAILED" : "\u{26A0} NOT JAILED";
      ...
      pi.on("session_start", async (_event: any, ctx: any) => {
        ctx.ui.setFooter((_tui: any, theme: any, footerData: any) => ({
          ...
          render(_width: number): string[] {
            ...
            const label = theme.fg(color, text);
            const gitPart = theme.fg("muted", branch ? branch : "no git");
            return [`''${label}  ''${gitPart}`];
          },
          dispose() {},
        }));
      });
  ...
  jailed-pi = jail-app {
    ...
    # below) reads this variable and recolors the footer when it is set.
