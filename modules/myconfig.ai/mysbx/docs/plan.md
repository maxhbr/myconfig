...
| Environment | `--clearenv`, forward `TERM COLORTERM LANG LC_ALL EDITOR VISUAL` when set, then `[env]`, then `HOME` and `PATH` (infrastructure, not overridable — `config.md` D14) — plus `TMUX_TMPDIR` on an interactive run with a `multiplexer`, infrastructure for the same reason (`config.md` D16/D17). The NixOS module additionally sets `RIPGREP_CONFIG_PATH` in the generated `[env]` (review-3 item 6): the `~/.config/ripgrep` mount alone is inert, the variable is the activation |
