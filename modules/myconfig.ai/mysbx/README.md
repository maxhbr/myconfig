...
whenever `myconfig.ai.mysbx.enable` is set. Its baseline grants the host tool
config this repo manages read-only (`~/.config/git`, `~/.config/ripgrep`,
`~/.config/bat`, and `~/.config/fish` when fish is enabled); per-agent
modules append their own config files to `myconfig.ai.mysbx.config.mounts`.
