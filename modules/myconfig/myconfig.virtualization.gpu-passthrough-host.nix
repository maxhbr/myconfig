...
in {
  options.myconfig = with lib; {
    virtualisation.gpuPassthroughHost.enable =
      mkEnableOption "myconfig.virtualisation.gpuPassthroughHost";
  };
  config = (lib.mkIf cfg.virtualisation.gpuPassthroughHost.enable ({
    ...
    environment = { systemPackages = with pkgs; [ list_IOMMU_groups ]; };
