...
let
  ...
  # this repo configures via `allProviders` above. This is done for EVERY
  # built-in provider, unconditionally; the repo's own providers are the
  # single source of truth for pi's model catalog.
  #
  ...
  providersExtension =
    let
      ...
      builtinIdsJson = builtins.toJSON builtinProviderIds;
    in
    pkgs.writeText "pi-providers.ts" ''
      ...
      // ALL of pi's built-in provider ids (see `builtinProviderIds` in the
      // generating Nix module). Registering `models: []` replaces each
      // provider's built-in model list, so only the repo-configured providers
      // above remain. Auth and `/login` entries for these providers keep
      // working; they just expose no models.
      const builtinProviderIds: string[] = ${builtinIdsJson};
