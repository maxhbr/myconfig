...
| Env forwarding | `try-fwd-env` list + `myconfig.ai.dev.jail.fwdEnvs`, always `OPENAI_API_KEY` | `envVars` attrset | same shape via `myconfig.ai.dev.nono.fwdEnvs` | pushed over the SSH session env at launch | `--env` / `--env-file` | none needed for model access | fixed allowlist (`FORWARDED_ENV_VARS`, lib.rs): terminal/locale block plus the `OPENAI_*`/`ANTHROPIC_*`/`OPENROUTER_*` credential block when set (bd myconfig-20j), plus the `[env]` table |
| Model credentials | real host key inside the sandbox | n/a | real host key inside the sandbox | real key, over SSH env | seeded config, endpoints rewritten to a sandbox-reachable proxy | **never reaches the guest** — host LiteLLM via bridge-only forwarder | host credentials forwarded through the fixed allowlist when set (`OPENAI_*`/`ANTHROPIC_*`/`OPENROUTER_*`, bd myconfig-20j) |
