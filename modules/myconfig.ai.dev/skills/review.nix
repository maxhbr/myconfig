...
# findings. Registered in the `myconfig.ai.dev.skills.handcraftedPrompts`
...
let
  cfg = config.myconfig.ai.dev.skills.review;
in
{
  options.myconfig.ai.dev.skills.review = with lib; {
    enable = mkEnableOption "myconfig.ai.dev.skills.review";
  };

  config = lib.mkMerge [
    # Enabled by default wherever pi-coding-agent is in use; a host can opt
    # out with `myconfig.ai.dev.skills.review.enable = false;`.
    { myconfig.ai.dev.skills.review.enable = lib.mkDefault true; }
    (lib.mkIf cfg.enable {
      # Expose the workflow as a pi prompt template (`/review`), deployed to
      # `~/.pi/agent/prompts/review.md` by `skills/default.nix`.
      myconfig.ai.dev.skills.handcraftedPrompts.review = ./review/prompt.md;
    })
  ];
}
