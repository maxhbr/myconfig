...
let
  cfg = config.myconfig.ai.dev.skills.implement-and-review-and-commit-until-done;
  skillDir = ./implement-and-review-and-commit-until-done;
in
{
  options.myconfig.ai.dev.skills.implement-and-review-and-commit-until-done = with lib; {
    enable = mkEnableOption "myconfig.ai.dev.skills.implement-and-review-and-commit-until-done";
  };

  config = lib.mkMerge [
    # Enabled by default wherever the skills framework is in use; a host can
    # opt out with
    # `myconfig.ai.dev.skills.implement-and-review-and-commit-until-done.enable = false;`.
    { myconfig.ai.dev.skills.implement-and-review-and-commit-until-done.enable = lib.mkDefault true; }
    (lib.mkIf cfg.enable {
      # Register the skill source; `skills/default.nix` applies it to every
      # enabled agent harness via the `handcrafted` registry.
      myconfig.ai.dev.skills.handcrafted.implement-and-review-and-commit-until-done = skillDir;
    })
  ];
}
