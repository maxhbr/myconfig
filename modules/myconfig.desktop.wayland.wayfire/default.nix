...
let
  ...
  #   wayfire-unwrapped = pkgs.wayfire;
  ...
  #     }
  #     export SDL_VIDEODwayfire=${
  #       config.environment.sessionVariables."SDL_VIDEODwayfire"
  #     }
  ...
  config =
    (lib.mkIf (cfg.desktop.wayland.enable && cfg.desktop.wayland.wayfire.enable) {
      home-manager.sharedModules = [{
        ...
        home.packages = with pkgs; [ wayfire-unstable ];
      }];
      myconfig.desktop.wayland.greetdSettings = {
        wayfire_session = {
          command = "${pkgs.wayfire-unstable}/bin/wayfire";
          user = "mhuber";
        };
      };
    });
