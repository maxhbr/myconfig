...
in
{
  ...
  config = (
    lib.mkIf (cfg.desktop.wayland.enable && builtins.elem "hikari" cfg.desktop.wayland.selectedSessions)
      {
        ...
        home-manager.sharedModules = [
          {
            xdg.configFile = {
              "hikari/hikari.conf".source =
                let
                  jsonFormat = pkgs.formats.json { };
                  conf = jsonFormat.generate "hikari.conf" cfg.desktop.wayland.hikari.settings;
                in
                conf;
