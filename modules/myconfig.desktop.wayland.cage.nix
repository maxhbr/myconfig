...
in {
  ...
  config = (lib.mkIf (cfg.desktop.wayland.enable
    && cfg.desktop.wayland.cage.enable && cfg.desktop.wayland.desktop == "") {
      # environment.systemPackages = with pkgs; [ cage firefox ];
      services.cage = {
        enable = true;
        inherit user;
        program = if cfg.desktop.wayland.cage.url != "" then
        ...
        environment = {
          ...
          (config.environment.variables ? GDK_PIXBUF_MODULE_FILE) {
