...
in {
  ...
  config = (lib.mkIf (cfg.desktop.wayland.enable && cfg.desktop.wayland.cage.enable && cfg.desktop.wayland.desktop == "") {
    # environment.systemPackages = with pkgs; [ cage firefox ];
    services.cage = {
      enable = true;
      inherit user;
      program = if cfg.desktop.wayland.cage.url != ""
                then "${pkgs.firefox}/bin/firefox -kiosk -private-window '${cfg.desktop.wayland.cage.url}'"
                else "${pkgs.foot}/bin/foot ${pkgs.tmux}/bin/tmux";
      environment = {
