...
let
  ...
  # Refresh script: read mtime of /run/current-system, compute age,
  # write a .prom file atomically into the textfile-collector dir.
  refreshScript = pkgs.writeShellApplication {
    ...
    text = ''
            ...
            {
              printf '# HELP nixos_system_activation_timestamp_seconds Unix time when /run/current-system was last switched.\n'
              printf '# TYPE nixos_system_activation_timestamp_seconds gauge\n'
              printf 'nixos_system_activation_timestamp_seconds %s\n' "$activation_ts"
              printf '# HELP nixos_system_age_seconds Age of the currently active NixOS generation in seconds.\n'
              printf '# TYPE nixos_system_age_seconds gauge\n'
              printf 'nixos_system_age_seconds %s\n' "$age_seconds"
              cat <<'EOF'
