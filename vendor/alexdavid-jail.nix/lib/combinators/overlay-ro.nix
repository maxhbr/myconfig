...
in
{
  ...
  doc = ''
    Creates a readonly overlay fs mount at a destination.
    ...
    Example:
    ```nix
    overlay-ro ["/source/foo" "/source/bar"] "/dest"
    ```
  '';
  impl =
    sources: dest:
    ...
    unsafe-add-raw-args (
      builtins.concatStringsSep " " (
        map (source: "--overlay-src ${escape source}") sources ++ [ "--ro-overlay ${escape dest}" ]
      )
    );
}
