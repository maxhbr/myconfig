...
#
# Because f13 no longer registers any localModels providers, the
# auto-generated model_list in modules/myconfig.ai/services.litellm.nix
# (a mkDefault) is empty. This file supplies the model_list directly
# with raw-name pass-through forward entries. The plain `=` (priority
# 100) still *merges* (concatenates) with other priority-100
# contributors — most importantly the skainet/ and trustedtokens/
# entries registered by the tng.nix flake modules (which also use a
# plain `=`). Previously this used `mkForce` (priority 50), which
# silently discarded those contributors.
#
...
{ ... }:
let
  # Model names published by thing's LiteLLM
  # (curl http://thing.wg0.maxhbr.local:4000/models | jq -r '.data[].id').
  models = [
    "gfx1151:Hy3-Q2_K_L"
...
in
{
  config = {
    services.litellm = {
      enable = true;
      settings.model_list = map mkForwardEntry models;
    };
  };
}
