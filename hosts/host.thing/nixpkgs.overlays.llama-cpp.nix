...
#
# --- Current state: pin DISABLED (2026-09-12) -------------------------------
# nixpkgs now ships llama.cpp v0.4.0 (2026-09-04, stable), which is NEWER
# than the previous pin b10549 (2026-08-21) and includes everything the pin
# was kept for: PR #25494 (Vulkan: dequant q8_0 KV once in coopmat1), the
# Qwen3.8 / MTP / ngram-mod / draft-dflash speculative functionality, and
# the qwen4exp architecture (PR #27742, merged 2026-08-27, first stable tag
# v0.4.0). The pin is therefore commented out and the host builds the stock
# nixpkgs llama-cpp.
#
# To jump AHEAD of nixpkgs again (e.g. pin a newer stable tag or a PR head):
#  1. uncomment the `nixpkgs.overlays` block below,
#  2. update `version` + `hash` (and `npmDepsHash` if tools/ui/package-lock.json
#     drifted) — recompute reproducibly with (see doc/qwen38-gfx1151/research-notes.md):
...
#  3. run the gfx1151 hardware build/benchmark gate before promoting any
#     candidate served by this build — see doc/qwen38-gfx1151/.
#
# The b10549 values are kept below so the previous pin can be restored
# verbatim.
#
# --- Version history --------------------------------------------------------
# b10056 (2026-07-16) was the first pin; it predates Qwen3.8-27B's release
...
#
# b10549 (2026-08-21) was the pin until 2026-09-12: the newest STABLE
# (non-prerelease) upstream tag including PR #25494 and the Qwen3.8 / MTP /
# ngram-mod / draft-dflash speculative functionality. It was chosen over the
# newer prereleases b10642/b10646 (2026-08-26/27) to minimise drift from the
# nixpkgs llama-cpp package.nix this overlay builds on top of (nixpkgs then
# shipped b10408; b10549 was +141 commits vs ~+590 for b10646), lowering the
# risk of an unbuildable combination that cannot be caught without the
# hardware.
{ ... }:
let
  # Previous pin (b10549). Keep in sync with the commented overlay block.
  version = "10549";
  ...
  # NOT BUILT since 2026-09-12: the only consumer was the (now disabled)
  # Qwen3.8-27B-DFlash2-Q6_K_XL candidate — see the commented entry in
  # hosts/host.thing/myconfig.ai.llama-cpp/Qwen3.8-27B.nix and
  # doc/TODOs/fix-dflash2-fork-abort-draft-output-weight.md. NOTE: the
  # fork has diverged further upstream since (213 ahead / 467 behind
  # master as of 2026-09-12); re-fetch rev/hash when re-enabling.
...
in
{
  # 2026-09-12: pin disabled — nixpkgs v0.4.0 is newer and covers
  # everything the pin provided (see header). Uncomment to re-pin.
  # nixpkgs.overlays = [
  #   (final: prev: {
  ...
  #     # top of the overlaid llama-cpp-vulkan (build infra +
