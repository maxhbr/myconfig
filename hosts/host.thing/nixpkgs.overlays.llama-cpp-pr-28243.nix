...
# with ggml-org/llama.cpp PR #28243 applied:
...
#
# Why a THIRD patched build (next to pr-27742 / pr-27754): the pinned
# PR-27742 build (`qwen4exp/qwen3.8-flash-next`) knows the `qwen4exp`
# trunk but does NOT load the NEXTN (MTP) tensors and has no MTP graph
# for the architecture — see the requirements section of
...
let
  ...
  # `tools/ui/package-lock.json` is byte-identical to b10408/b10549
  # (verified by fetching and hashing all three), so the nixpkgs
  # npmDepsHash still applies.
