...
#
# Partition, format and mount the NixOS target disk for the NanoPC-T6
...
# by-label devices in hosts/host.t6/hardware-configuration.nix, then mounts
# them at /mnt and /mnt/boot.
#
# NVMe is the preferred target because it leaves the vendor eMMC untouched.
# If targeting eMMC, note that the EDK2 microSD must remain inserted for
# firmware; eMMC here is just the NixOS root filesystem.
#
# Usage:
...
Usage: $0 <device>
  ...
  <device>   Whole-disk target for NixOS (e.g. /dev/nvme0n1, /dev/mmcblk0).
