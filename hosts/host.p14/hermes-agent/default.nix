{ config, lib, pkgs, inputs, ... }: 
  let
    ...
    apiServerHost = if cfg.container.enable
                    then config.containers.hermes.localAddress
                    else "localhost";
    hermesServiceCfg = {
      enable = true;
      user = "mhuber";
      ...
      environmentFiles = let
        hermes-api-env = (pkgs.writeText "hermes-api-env" ''
  ...
  in
  {
    ...
    config = lib.mkIf cfg.enable {
      ...
      home-manager.users.mhuber = { pkgs, ... }: {
        home.packages = [
