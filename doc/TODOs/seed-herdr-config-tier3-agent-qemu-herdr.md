# Seed the `herdr` config in the tier-3 `agent-qemu-herdr` guest
...
- **Tier 3** (`modules/myconfig.ai/fns/seed-agent-config.nix`,
  `agentConfigPaths`) has **no `herdr` entry**, so `agent-qemu-herdr`
...
`modules/myconfig.ai/agent-qemu-herdr.README.md` ("Agent-configuration seeding")
...
1. **Close the gap** (preferred): add a `herdr` entry to `agentConfigPaths` in
   ...
   `doc/agent-qemu-herdr-vs-agent-microvm-herdr.md` to say the sets match again.
...
- Option 1: run `agent-qemu-herdr` in a project directory on a KVM host and
